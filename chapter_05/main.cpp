...
vec3 color(const ray& r, hitable *world, int depth){
    ...
    // do not know why, but the color we used is as mask
    // return vec3(0, 0, 0) will be a total black image
...
hitable *earth() {
  ...
  return new sphere(vec3(0, 0, 0), 2.5, mat);
}
...
int main(){
  ...
  camera cam(vec3(0,0,4),vec3(0,0,0),vec3(0,1,0),90,float(nx)/float(ny));
